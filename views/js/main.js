...
/*
  Project 4
  Reformatted (to allow printing and easier study)
  and optimized by Leon Tabak.
  30 Jul 2015

  Find changes that Leon Tabak made to improve
  the performance of this program by searching
  for "LHT".
*/

/*
Welcome to the 60fps project! Your goal is to make Cam's 
Pizzeria website run jank-free at 60 frames per second.

There are two major issues in this code that lead to 
sub-60fps performance. Can you spot and fix both?


Built into the code, you'll find a few instances of the 
User Timing API (window.performance), which will be 
console.log()ing frame rate data into the browser console. 
...
// add object to hold global variables---LHT 30 Jul 2015
var global = {};

// As you may have realized, this website randomly 
// generates pizzas.
...
function updatePositions() {
  ...
  // made items a global variable---LHT 30 Jul 2015
  //var items = document.querySelectorAll('.mover');
  var scaledYcoordinate = document.body.scrollTop/1250;
  // create array of sine values outside of loop---LHT 30 Jul 2015
    var scaledSines = [100 * Math.sin(scaledYcoordinate + 0),
               100 * Math.sin(scaledYcoordinate + 1),
               100 * Math.sin(scaledYcoordinate + 2),
               100 * Math.sin(scaledYcoordinate + 3),
               100 * Math.sin(scaledYcoordinate + 4),
  		];

  // create variable before entering loop
  var phase;
  var j = 0;
  var item;
  ...
  for (var i = 0; i < length; i++) {
      ...
      // removed y calculation from loop---LHT 30 Jul 2015
      //phase = Math.sin( scaledYcoordinate + j);
      //phase = sines[j];

      item = global.items[i];
      //item.style.left = item.basicLeft + 100 * phase +
      //  'px';
      item.style.left = (item.basicLeft + scaledSines[j]) + 'px';

      // replace a division (i % 5) with an addition---LHT 30 Jul 2015
      j++;
      if( j === 5 ) {
	  j = 0;
      } // if
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // precompute left coordinates---LHT 30 Jul 2015
  var leftCoordinates = [];
  for( var j = 0; j < cols; j += 1 ) {
      leftCoordinates.push((j % cols) * s);
  } // for

  // change i < 200 to i < 24
  // this changes number of pizzas
  // screen is not big enough to hold 200 pizzas
  // LHT 30 Jul 2015
  var j = 0;
  var yCoordinate = 0;
  ...
  for (var i = 0; i < numberOfPizzas; i++) {
    ...
    // use precomputed values---LHT 30 Jul 2015
    //elem.basicLeft = (i % cols) * s;
    elem.basicLeft = leftCoordinates[j];

    // replace a division (i / cols) with an addition---LHT 30 Jul 2015
    j++;
    if( j > cols ) {
	j = 0;
        yCoordinate += s;
